  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        // Use Request objects to avoid weird caching modes on some browsers
...
// - navigation: cache-first for index.html (offline shell)
...
self.addEventListener("fetch", (event) => {
  ...
  // Keep your update banner accurate
  ...
  if (url.origin === self.location.origin && isNavigationRequest(req)) {
    event.respondWith(
      (async () => {
        ...
        try {
          // Try network first so normal refresh stays current
