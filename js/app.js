...
obRoutineCreateBtn?.addEventListener("click", ()=>{
  ...
  if(tpl === "upperlower"){
    ...
    base = defaultRoutine(); // your existing PPL seed
  } else {
    // blank routine
    base = defaultRoutine();
    base.name = "Routine";
  ...
  // ✅ make onboarding routine the default immediately
  setActiveRoutine(base.id);
  renderRoutineDropdown(base.id);

  // open editor (still customizable)
...
function renderHeaderSub(){
  ...
  sub.textContent = `Minimal tracker • Protein goal ${profile.proteinGoal}g • ${restTxt}`;
}
...
function refreshHome(){
  ...
  if(homeProteinCircle){
    homeProteinCircle.style.background = `conic-gradient(var(--good) ${deg}deg, #202124 0deg)`;
  }
  ...
  // focus text (simple logic: pick label of today, or PR suggestion)
  if(homeFocus){
    if(day.rest){
      homeFocus.textContent = "Recovery + mobility. Light walk if you feel good.";
    } else if(label){
      homeFocus.textContent = `Focus: ${label}`;
    } else {
      homeFocus.textContent = "Focus: execute clean sets + progressive overload.";
    }
  }
}
...
function showScreen(name){
  ...
  // refresh only what’s needed when a screen becomes visible
  if(name === "home"){
    refreshHome();
  }
  if(name === "weight"){
...
function setBWView(next){
  ...
  if(bwView === "table"){
    bwTableWrap.style.display = "";
    bwChartWrap.style.display = "none";
    bwViewTableBtn.classList.remove("ghost");
    bwViewGraphBtn.classList.add("ghost");
  } else {
    bwTableWrap.style.display = "none";
    bwChartWrap.style.display = "";
...
function renderBW(){
  ...
  bwLatest.textContent = latest ? `${latest.bw.toFixed(1)} lbs (${latest.date})` : "—";
  ...
  bwAvg.textContent = avg ? `${avg.toFixed(1)} lbs` : "—";
...
function renderProteinTotals(){
  ...
  const rem = getProteinGoal() - total;
  ...
  if(rawQ){
    ...
    if(!exists){
      ...
      create.addEventListener("click", ()=>{
        const created = cleanExerciseName(rawQ);
...
const rtExSuggestions = document.getElementById("rtExSuggestions");
  ...
  rtAddExName.addEventListener("focus", ()=>{
  openExercisePicker({
...
// 🔹 NAVIGATION WIRES HERE (only once)
bindNavigation();

// ✅ new onboarding routing
hydrateOnboardingInputs();
routeInitialScreen();
